...
//#define STK_LT_N_CT	90
//#define STK_HT_N_CT	110

#define STK_MAX_MIN_DIFF	180
#define STK_LT_N_CT	90
#define STK_HT_N_CT	110

//#define STK_MAX_MIN_DIFF	220
//#define STK_LT_N_CT	160
//#define STK_HT_N_CT	200
...
#define STK_DEF_THDH 1000
#define STK_DEF_THDL 800
uint16_t ps_low_def_thd  = 800;
...
#if 1
	if(obj->psi_set && obj->psi_set>obj->psi) {
		obj->psi_set = obj->psi;
		APS_LOG("%s: new psi: psi=%d \n", __func__, obj->psi);	
		obj->psa = obj->psi;
		return 0;
	}
#endif
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_show_led(struct device_driver *ddri, char *buf)
{
	ssize_t res;
	if(!stk3x1x_obj)
	{
	...
	res = scnprintf(buf, PAGE_SIZE, "0x%02X\n", stk3x1x_obj->ledctrl_val);     
	return res;    
}
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_store_led(struct device_driver *ddri, const char *buf, size_t count)
{
    int led;
    struct i2c_client *client;
	client = stk3x1x_i2c_client;
	...
	if(1 == sscanf(buf, "0x%x", &led))
	{
		stk3x1x_obj->ledctrl_val = led;
		stk3x1x_write_led(client, stk3x1x_obj->ledctrl_val);
	}
	else 
	{
		APS_ERR("invalid content: '%s', length = %d\n", buf, (int)count);
...
static DRIVER_ATTR(led,   S_IWUSR | S_IRUGO, stk3x1x_show_led, stk3x1x_store_led);
