			   ...
			   unsigned long *nr)
{
    ...
    if (vmscan_swap_file_ratio) {
		...
		if (time_after(jiffies, t + vmscan_duration_ms/1000 * HZ)) {
			...
			if( ((SwapinCount-history[0] + SwapoutCount - history[1]) / (jiffies-t) * HZ) > vmscan_threshold){
				bThrashing = true;
			       ...
			       unsigned long *nr_attempted)
{
	...
	/* Reclaim above the 2 * high watermark. */
	sc->nr_to_reclaim = max(SWAP_CLUSTER_MAX, high_wmark_pages(zone) << 1);
#else
