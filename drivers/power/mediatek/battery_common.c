...
{
	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] wake_up_bat2. \r\n");
...
{
	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] wake_up_bat3. \r\n");
...
{
       INT32 tmp=battery_meter_get_charging_current();
	   INT32 ICharging = battery_meter_get_battery_current();

		printk("show_charging_current_value ICharging= %d,tmp=%d\n",
               ICharging,tmp);
               
    battery_xlog_printk(BAT_LOG_CRTI, "show_charging_current_value ICharging= %d,tmp=%d\n",
               ICharging,tmp);      
...
{
//     sscanf(buf, "%u", &g_call_state);
    battery_xlog_printk(BAT_LOG_CRTI, "store_charging_current_value\n");    
    return size;
}
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		{
			battery_xlog_printk(BAT_LOG_CRTI, "[Sync_Real] chr_wake_up_bat=1 , do not update UI_SOC\n");		
		}
#else
		BMT_status.UI_SOC--;
#endif
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (adjust_power != -1) {
        ...
        battery_xlog_printk(BAT_LOG_CRTI, "battery_update.\n");

#ifdef DLPT_POWER_OFF_EN
    extern int dlpt_check_power_off(void);
    if(bat_data->BAT_CAPACITY <= DLPT_POWER_OFF_THD)
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[DLPT_POWER_OFF_EN] run\n");
        
        if(bat_data->BAT_CAPACITY==0)
        {
            bat_data->BAT_CAPACITY=1;
            battery_xlog_printk(BAT_LOG_CRTI, "[DLPT_POWER_OFF_EN] SOC=0 but keep %d\n", bat_data->BAT_CAPACITY);
        }
        if(dlpt_check_power_off()==1)
        {
            bat_data->BAT_CAPACITY=0;
            battery_xlog_printk(BAT_LOG_CRTI, "[DLPT_POWER_OFF_EN] SOC=%d to power off\n", bat_data->BAT_CAPACITY);
        }
    }
    else
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[DLPT_POWER_OFF_EN] disable(%d)\n", bat_data->BAT_CAPACITY);
    }
#endif
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE+5) {
...
static void mt_battery_notify_VCharger_check(void)
{
	...
	#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
	if (BMT_status.charger_vol > V_CHARGER_MAX || BMT_status.charger_protect_status == charger_OVER_VOL) {
	#else
	if (BMT_status.charger_vol > v_charger_max || BMT_status.charger_protect_status == charger_OVER_VOL) {
	#endif
...
#endif
#if 0
	if (g_platform_boot_mode == META_BOOT && upmu_is_chr_det() == KAL_FALSE) {
		battery_xlog_printk(BAT_LOG_CRTI,
					"[bat_thread_kthread] Unplug Charger/USB In Meta Mode!\r\n");
		msleep(2000);
		battery_xlog_printk(BAT_LOG_CRTI,
					"[bat_thread_kthread] Shutdown OS!\r\n");
		battery_charging_control(CHARGING_CMD_SET_POWER_OFF, NULL);
	}
#endif
}
...
int bat_thread_kthread(void *x)
{
	...
	while (1) {
		...
		if(fg_wake_up_bat==KAL_TRUE)
		{
			...
			battery_xlog_printk(BAT_LOG_CRTI, "unlock battery_fg_lock \n");
		}
#endif //#ifdef FG_BAT_INT
...
void check_battery_exist(void)
{
	...
	if(power_supply_flag == 1)
	{
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Disable check battery exist power_supply_flag\n");
		return ;
	}
	...
	if (BMT_status.charger_exist == KAL_TRUE)
	{
		if (chargin_hw_init_done && upmu_is_chr_det() == KAL_TRUE)
		{
			...
			for(i=0;i<4;i++)
			{
				enable=pmic_get_register_value(PMIC_RG_CHR_EN);
					...
					if(enable==1)
					{
						...
						{
							cnt++;
							battery_xlog_printk(BAT_LOG_CRTI,"[charger_hv_detect_sw_thread_handler] fail ICharging=%d , VCHR=%d cnt=%d \n",ICharging,VCharger,cnt);	
						}
						else
						{
							battery_xlog_printk(BAT_LOG_CRTI,"[charger_hv_detect_sw_thread_handler] success ICharging=%d , VCHR=%d cnt=%d \n",ICharging,VCharger,cnt);	
							break;
						}
					}
					else
			...
			if (cnt>=3)
			{
				...
				battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
				battery_xlog_printk(BAT_LOG_CRTI,"[charger_hv_detect_sw_thread_handler] ICharging=%d VCHR=%d cnt=%d turn off charging\n",ICharging,VCharger,cnt);					
			}	
...
int charger_hv_detect_sw_thread_handler(void *unused)
		{
	ktime_t ktime;
	kal_uint32 hv_voltage = BATTERY_VOLT_08_500000_V;//V_CHARGER_MAX*1000;
