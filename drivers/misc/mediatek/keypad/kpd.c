...
#define AEON_FCOVER_SUPPORT

#define GPIO_FCOVER_EINT_PIN  (GPIO1 | 0x80000000)
#define GPIO_EINT_HALL_PIN_M_GPIO GPIO_MODE_00
#define GPIO_EINT_HALL_PIN_M_EINT GPIO_MODE_07
...
//static void fcover_open_close_handler(struct work_struct *work);
//static DECLARE_WORK(fcover_able, fcover_open_close_handler);
...
//static u8 fcover_key_state =1;

//#define KERNEL_OPEN_FCOVER

//spinlock_t fcover_lock = SPIN_LOCK_UNLOCKED;

static DEFINE_SPINLOCK(fcover_lock);

int fcover_close_flag=1; // fenggy test

#ifdef KERNEL_OPEN_FCOVER
extern int lcm_backlight_flag;
#endif

//extern void himax_cover_enable(int open);


//static void fcover_open_close_handler(struct work_struct *work)
//{
//     himax_cover_enable(fcover_close_flag);
//}

//extern void tpd_fcover_setting(u8 enable); 

static int new_fcover=1;
...
//     tasklet_schedule(&kpd_fcover_tasklet);
       ...
       mt_eint_mask(FCOVER_KEY_EINT);  
       printk(KPD_SAY "kpd_fcover_eint_handler ..\n");

       queue_work(fcover_workqueue, &fcover_work);     
//     if(!ret)
//     {
//             printk("[fcover]kpd_fcover_eint_handler return:%d!\n", ret);                    
//     }
...
long kpd_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SET_HALL_STATE:
		...
		printk("==========fcover_close_flag(%d)========\n",fcover_close_flag);
		if(copy_to_user(uarg, &fcover_close_flag, sizeof(fcover_close_flag)))
		{
			return -EFAULT;
			//break;
		}				 
